...
const shortMovies = movies.filter (x => x.title.length <= 10); 

const shortMoviesTitles = shortMovies.map(x => x.title);
  
console.log(shortMoviesTitles);

console.log("-----Long Movies");

const longMovies = movies.filter (x => x.title.length > 10);

const longMoviesTitles = longMovies.map(x => x.title);

console.log(longMoviesTitles);

console.log("Number of Movies between 1980-1989");

const childhoodMovies = movies.filter(x => x.year >= 1980 && x.year <= 1989);

const childhoodMoviesYear = childhoodMovies.map(({title, year}) => ({title, year}));
console.log(childhoodMoviesYear);

console.log("The nubmber of movies between 1980-1989 is " + childhoodMovies.length);

console.log("-----Extra Key Tag");
...
movies.forEach(x => x.Tag = getRatingTag(x.rating));

console.log(movies);

console.log("Using Chaining");

const highRatedMovies = movies.filter(z => z.rating > 6)
.map(z => z.rating);

console.log(highRatedMovies);

console.log("------Number 6 count total bumber of movies");

const keywordMovies = movies.filter(y => y.title.toLowerCase().includes("surfer") ||
          y.title.toLowerCase().includes("alien") ||  y.title.toLowerCase().includes("benjamin"));

console.log(keywordMovies);         
console.log(keywordMovies.length);

console.log("-----No7 Duplicates");

let numberOfDuplicates = [];

for (var i = 0; i < movies.length; i++) {
  const splitMovieTitle = movies[i].title.toLowerCase().split(" ");

  for (var z = 0; z < splitMovieTitle.length; z++) {
    const wordInTitle = splitMovieTitle[z];
    const newMovieTitle = splitMovieTitle.filter((otherWords) => otherWords === wordInTitle);
    const similarWords = newMovieTitle.length;

    if (similarWords > 1) { 
      if (numberOfDuplicates.includes(movies[i].title)) {

      }
      else {numberOfDuplicates.push(movies[i].title)};
    };
...
const averageRating = movies.map(x => x.rating);

//function calculateAverage (anything) {
  //let sumRate = 0;
  //let rateLength = anything.length;
  //for (var j = 0; j < rateLength; j++) {
   // sumRate = sumRate + anything[j];
 // }
 // return sumRate / rateLength;
//}

const average = averageRating.reduce(function(sum, value) {
 return sum + value;
...
console.log("-----Count number of good/bad movies"); // couldn't solve it

//console.log(movies[5]);

const moviesTag = movies.map(z => z.Tag);

const goodMovies = moviesTag.reduce((accumulator, currentValue) => accumulator.concat(currentValue),
