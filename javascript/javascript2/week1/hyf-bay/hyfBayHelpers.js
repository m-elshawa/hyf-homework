...
//testProductNames.forEach(function(item) {
// const lista = document.createElement("li");
//   const text = document.createTextNode(item);
// lista.appendChild(text);
//ulProducts.appendChild(lista);
...
//const namesMore = getAvailableProducts().map(x => x.name);

//renderProducts(namesMore);

//function shitMe(crap) {
  //var vals = [];
  //for (var i = 0; i < crap.length; i++) {
    //var name = crap[i].name;
    //var rating = crap[i].rating;
    //var price = crap[i].price;
    //var destination = crap[i].shipsTo;
    //var combinedInfo = name + " | " + price + " | " + rating + " | " +  destination;
    
    //vals.push(combinedInfo);
  //}
  //return vals;
//}
//const hitHim = shitMe(getAvailableProducts());
//renderProducts(hitHim);
